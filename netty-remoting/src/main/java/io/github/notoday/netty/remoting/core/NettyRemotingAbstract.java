...
public abstract class NettyRemotingAbstract {
    ...
    public void processRequestCommand(ChannelHandlerContext ctx, RemotingCommand request) throws Exception {
        ...
        if (pair == null) {
            response(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.REQUEST_CODE_NOT_SUPPORTED, "[REQUEST_CODE_NOT_SUPPORTED] request code " + request.getCode() + " not supported."));
            return;
        }

        NettyRequestProcessor requestProcessor = pair.getObj1();
        if (requestProcessor.rejectRequest()) {
            response(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.COMMAND_NOT_AVAILABLE_NOW, "[COMMAND_UNAVAILABLE_NOW] this command is currently unavailable."));
            return;
        }
        ...
        RequestTask task = new RequestTask(() -> {
            try {
                ...
                // 单向消息不需要响应
                if (!oneway) {
                    response(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.SYSTEM_ERROR, e.getMessage()));
                }
            }
        }, ctx.channel(), request);
        ...
        try {
            ...
            // 单向消息不需要响应
            if (!oneway) {
                response(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.SYSTEM_BUSY, "[OVERLOAD] system busy, try later."));
            }
        }
    }
