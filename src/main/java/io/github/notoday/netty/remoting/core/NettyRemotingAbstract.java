...
public abstract class NettyRemotingAbstract {
    ...
    public void processMessageReceived(ChannelHandlerContext ctx, RemotingCommandProtobuf msg) throws Exception {
        if (msg == null) return;
        RemotingCommand cmd = RemotingCommand.of(msg);
        if (cmd.isResponse()) {
    ...
    public void processRequestCommand(ChannelHandlerContext ctx, RemotingCommand request) throws Exception {
        ...
        if (pair == null) {
            failure(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.REQUEST_CODE_NOT_SUPPORTED, "[REQUEST_CODE_NOT_SUPPORTED] request code " + request.getCode() + " not supported."));
            return;
        }

        NettyRequestProcessor requestProcessor = pair.getObj1();
        if (requestProcessor.rejectRequest()) {
            failure(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.COMMAND_NOT_AVAILABLE_NOW, "[COMMAND_UNAVAILABLE_NOW] this command is currently unavailable."));
            return;
        }
        ...
        RequestTask task = new RequestTask(() -> {
            try {
                ...
                // 单向消息不需要响应
                if (!oneway) {
                    failure(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.SYSTEM_ERROR, e.getMessage()));
                }
            }
        }, ctx.channel(), request);
        ...
        try {
            ...
            // 单向消息不需要响应
            if (!oneway) {
                failure(ctx, RemotingCommand.failure(reqId, RemotingSysResponseCode.SYSTEM_BUSY, "[OVERLOAD] system busy, try later."));
            }
        }
    }

    private void failure(ChannelHandlerContext ctx, RemotingCommand response) {
        ctx.writeAndFlush(response.protobuf());
    }
    ...
    protected RemotingCommand invokeSyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis) throws InterruptedException, RemotingConnectException, RemotingSendRequestException, RemotingTimeoutException {
        ...
        channel.writeAndFlush(request.protobuf()).addListener(future -> {
            if (future.isSuccess()) {
